...
const int MAX_BITRATE = 3000000; // 30Mbits/s

// Image variables
int nCount = 0;
IplImage *R, *G, *B, *image, *imgHSV, *bw;
Mat img, grey;
char key;
...
static void video_buffer_callback(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer)
{
    ...
    if (pData)
    {
        ...
        if (buffer->length)
        {
            ...
            //****************************\\
            //  Gustavo : OPEN CV Stuff   \\
            //****************************\\
            int w = pData->pstate->width; // get image size
            int h = pData->pstate->height;

            memcpy(B->imageData, buffer->data, w * h); // read B
            memcpy(G->imageData, buffer->data + w * h, w * h); // read G
            memcpy(R->imageData, buffer->data + w * h + w * h, w * h); // read R

            cvMerge(R, G, B, NULL, image);

            //cvCvtColor(image, image, CV_YCrCb2BGR);
	        cvCvtColor(image, bw, CV_BGR2GRAY);
...
static MMAL_COMPONENT_T *create_camera_component(RASPIVID_STATE *state)
{
    ...
    // Gustavo - Using RGB encoding
    format->encoding_variant = MMAL_ENCODING_BGR24; //  MMAL_ENCODING_I420;
    format->encoding = MMAL_ENCODING_BGR24;         //  MMAL_ENCODING_I420;
...
int main(int argc, char *argv[])
{
    if (argc > 1)
    {
        ...
        if (argc > 7)
        {
            ...
            Smax = atoi(argv[6]);
            Vmax = atoi(argv[7]);
            if (argc > 8) debug = (atoi(argv[8]) == 1 ) ? true : false;
        }
    }
    ...
    // Gustavo - Initiate Variables
    cvNamedWindow("Video", CV_WINDOW_AUTOSIZE);
    if (debug) cvNamedWindow("Black", CV_WINDOW_AUTOSIZE);
    bw = cvCreateImage(cvSize(FRAME_WIDTH, FRAME_HEIGHT), IPL_DEPTH_8U, 1);      // Black and White frame
    R = cvCreateImage(cvSize(FRAME_WIDTH, FRAME_HEIGHT), IPL_DEPTH_8U, 1);      // R component of YUV I420 frame
    G = cvCreateImage(cvSize(FRAME_WIDTH, FRAME_HEIGHT), IPL_DEPTH_8U, 1);      // G component of YUV I420 frame
    B = cvCreateImage(cvSize(FRAME_WIDTH, FRAME_HEIGHT), IPL_DEPTH_8U, 1);      // B component of YUV I420 frame
    ...
    grey = cvCreateImage(cvSize(FRAME_WIDTH, FRAME_HEIGHT), IPL_DEPTH_8U, 3);
