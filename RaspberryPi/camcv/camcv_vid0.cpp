...
int nCount = 0;
IplImage *py, *pu, *pv, *pu_big, *pv_big, *image, *dstImage, *imgHSV, *imgTracking;
...
{
    string instruction = "Move forward";
...
void trackFilteredObject(IplImage *imgThresh, Mat &cameraFeed)
{
    ...
    if (area > 200)
    {
        ...
        putText(cameraFeed, "No object found, use random algorithm", Point(20,20), 1, 1, Scalar(255,255,0));
    }
    free(moments);
}
    ...
    if (pData)
    {
        ...
        if (buffer->length)
        {
            ...
            if (pData->pstate->graymode == 0)
            {
                ...
                trackFilteredObject(imgThresh, img);
                if (debug)
                {
